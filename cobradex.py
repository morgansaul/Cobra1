...
class DEXScanner:
    ...
    def _scan_approvals(self):
        ...
        approval_risks = [
            (r"(approve|increaseAllowance)\([^)]*,\s*(0x[fF]{64}|ethers\.MaxUint|2\*\*256-1)", 
...
def detect_chain(url: str) -> str:
    ...
    url = url.lower()
    if 'arbitrum' in url or 'arb/' in url or 'chain=42161' in url:
        return 'arbitrum'
    if 'optimism' in url or 'op/' in url or 'chain=10' in url:
        return 'optimism'
    if 'polygon' in url or 'matic/' in url or 'chain=137' in url:
        return 'polygon'
    if 'bsc' in url or 'binance/' in url or 'chain=56' in url:
        return 'bsc'
    if 'avalanche' in url or 'avax/' in url or 'chain=43114' in url:
        return 'avalanche'
    return 'ethereum'
...
def main():
    ...
    parser.add_argument('-c', '--chain', help='Force chain (ethereum/arbitrum/optimism/polygon/bsc/avalanche)')
