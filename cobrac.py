...
# ===== Config =====
ETHERSCAN_API_KEY = "MDMC7E2W745BSYW7ZR579EWNEP7MTM5AWX"  # Replace with actual key
...
def detect_token_standard(abi: Dict) -> str:
    """Detect if contract is ERC-20, ERC-721 or other"""
    functions = [item["name"].lower() for item in abi if item["type"] == "function"]
    
    # ERC-20 check
    erc20_functions = {'transfer', 'transferfrom', 'approve', 'allowance', 'balanceof', 'totalsupply'}
    if erc20_functions.issubset(set(functions)):
        return "ERC-20"
    
    # ERC-721 check
    erc721_functions = {'safeTransferFrom', 'transferFrom', 'approve', 'ownerOf', 'balanceOf'}
    if erc721_functions.issubset(set(functions)):
        return "ERC-721"
    ...
    try:
        ...
        for contract in slither.contracts:
            for func in contract.functions:
                if func.name.lower() in ["transfer", "approve", "mint", "burn"]:
...
def check_reentrancy_risk(abi: Dict) -> bool:
    ...
    for item in abi:
        if item["type"] == "function" and "modifiers" in item:
            if any("nonReentrant" in mod for mod in item["modifiers"]):
                return False
    return True

def check_centralization_risks(contract_address: str, abi: Dict) -> Dict:
    """Check for owner/admin privileges"""
    risks = {
        "owner_functions": [],
        "admin_roles": []
    }
    for item in abi:
        if item["type"] == "function":
            if "onlyOwner" in str(item):
                risks["owner_functions"].append(item["name"])
            if "onlyAdmin" in str(item) or "onlyRole" in str(item):
